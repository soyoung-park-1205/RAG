...
def get_relevance_check_format():
    response_schemas = [
        ResponseSchema(name="relevance", description="연관성이 있을 경우 1, 없을 경우 0")
    ]
...
def build_context_prompt():
    ...
    당신은 뉴스 검색 결과를 참고하여 사용자의 질문에 답변하는 AI입니다.

    다음은 최근의 '{keyword}' 관련 검색 결과 내용입니다:
    ...
    - 답변은 반드시 위의 검색 내용 만을 활용할 것. (검색 내용에 언급된 명사 만을 활용하여 답변)
    - "~니다."와 같은 친절한 격식체를 사용하여 한국어로 간결하게 답변할 것.
    - "내가 대답한다면, ", "이렇게 답변해보세요." 등의 표현은 절대 사용하지 말 것.
    - 검색 내용에 없는 내용을 절대 만들어 내지 말 것.
...
def build_doc_relevance_prompt():
    return PromptTemplate.from_template(
        """
        {document} 내에 있는 내용이  
        {question} 질문과 관련이 있는지 여부를 리턴해주세요.
        관련 있을 경우 1, 관련 없을 경우 0을 리턴해주세요.
